...
export function PlayerForm() {
  const { players, updatePlayers, checkNameAvailability, setErrorToast } =
    usePlayerContext();
  ...
  const playerSubmit = () => {
    ...
    if (checkNameAvailability(playerName)) {
      ...
      setErrorToast({
        state: true,
        message: `'${playerName}' is already taken, please choose another name`,
      });
    }
  };
        ...
        <Form onSubmit={() => playerSubmit()}>
          ...
          <div className={"text-center"}>
            <Button
              ...
              onMouseOut={(e) => toggleVariant(e.currentTarget)}
              onMouseOver={(e) => toggleVariant(e.currentTarget)}
              className={"w-75 border-success border-3"}
              type={"submit"}
...
function ErrorToast() {
  ...
  return (
    <Toast
      ...
      onClose={() => setErrorToast({ state: false, message: "" })}
      show={errorToast.state}
      delay={3000}
      autohide
    >
      <Toast.Body>{errorToast.message}</Toast.Body>
    </Toast>
  );
}
