      ...
      <div>
        ...
        <Row className="pt-5">
          <Col className={"my-auto"}>
            ...
            <UnoCard
              card={{
                cardColour: "black",
                cardText: currentDeck.length > 0 ? currentDeck.length : "#",
              }}
              className={"deckCard user-select-none"}
              onClickFunction={() => drawCard(turn, 1, false, true)}
            />
          </Col>
          <Col className={"my-auto"}>
            ...
            <UnoCard
              card={{
                cardColour: "black",
                cardText: playedDeck.length > 0 ? playedDeck.length : "#",
              }}
        ...
        <Container>
          {players.map((player, i) => (
            <p
              key={i}
              className={"text-white justify-content-between text-center"}
            >
              {player.calledUno ? `${player.name} called uno` : ""}
            </p>
          ))}
        </Container>
      </div>
...
function ColourModal() {
  ...
  return (
    <Modal show={colourModalShow.state} onHide={() => console.log("hide")}>
      <Modal.Header>
      ...
      <div className={"p-5"}>
        <div className={"d-flex justify-content-center"}>
          <div
            ...
            className={"red colourPicker"}
            onClick={() => pick("red")}
          />
          <div
            style={{ height: "150px", width: "150px" }}
            className={"blue colourPicker"}
            onClick={() => pick("blue")}
          />
        </div>
        <div className={"d-flex justify-content-center"}>
          <div
            style={{ height: "150px", width: "150px" }}
            className={"yellow colourPicker"}
            onClick={() => pick("yellow")}
          />
          <div
            style={{ height: "150px", width: "150px" }}
            className={"green colourPicker"}
            onClick={() => pick("green")}
          />
        </div>
      </div>
    ...
    <>
      <div>
        ...
        <div className="d-flex" style={{ overflowX: "scroll" }}>
          {currentPlayer.cards.map((card, j) => (
            <UnoCard
              card={card}
              key={j}
...
function TurnScreen() {
  const { currentPlayer, nextTurn, colourModalShow, callUno, winner } =
    useGameContext();
  return (
    <>
...
function YourTurnScreen() {
  ...
  return (
    <Container>
      <h1
        className={"text-center text-white mb-3 text-uppercase"}
      >{`${currentPlayer.name}'s turn`}</h1>
      <div className={"text-center"}>
        <Button
          onClick={() => setYourTurnScreen(false)}
          style={{ width: "250px" }}
        >
          Start turn
        </Button>
      ...
      {unoPlayers.length > 0 && (
        <>
          <h3 className={"text-center text-white mb-3 text-uppercase"}>
            Forgot to say uno?
          </h3>
          <Container>
            {unoPlayers.map((player, i) => (
              <Button
                key={i}
                className={"d-block mx-auto"}
                style={{ width: "250px" }}
                onClick={() => reportUno(player.turnID)}
              >
                {player.name}
              </Button>
            ))}
